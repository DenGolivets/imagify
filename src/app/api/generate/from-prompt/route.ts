...
import { fromPromptSchema } from "@/lib/validations/generate";
import { generateRateLimit } from "@/utils/rateLimit";
import { generateFromPrompt } from "@/lib/ai/gemini";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validatedFields = fromPromptSchema.safeParse(body);
    ...
    try {
      resultImageBase64 = await generateFromPrompt(prompt);
    } catch (error: unknown) {
